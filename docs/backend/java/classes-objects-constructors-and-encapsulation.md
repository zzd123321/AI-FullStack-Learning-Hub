...
```java
LearningAccount account = new LearningAccount("小林", 300);
```

它大致经历四步：JVM 为对象状态分配空间；字段先取得默认值；构造方法校验参数并建立初始状态；构造成功后，把指向对象的引用交给 `account`。如果构造方法抛出异常，调用方拿不到一个“创建了一半”的正常对象。

```text
栈中的局部变量 account ──引用──→ 堆中的 LearningAccount
                                  name = "小林"
                                  weeklyGoal = 300
                                  studiedMinutes = 0
```

因此，构造方法最重要的职责不是“给字段赋值”这句表面动作，而是建立对象从出生起就必须满足的不变量。例如目标分钟数必须为正；如果允许先创建非法对象、再期待调用方补一次 setter，那么任何漏调路径都会把非法状态带进后续业务。

JavaScript 对象常可随时增加或删除属性；Java 类的字段结构在编译时已经确定。这样的约束略显啰嗦，却让 IDE、编译器和维护者都能知道对象允许有哪些状态变化。封装也不是拒绝变化，而是把变化集中到能守住规则的方法中。
