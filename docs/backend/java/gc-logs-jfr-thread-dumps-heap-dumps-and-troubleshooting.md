...
用户说“接口很慢”时，至少有几条不同因果链：CPU 忙导致执行慢、锁或下游导致等待、GC 暂停、请求排队，甚至只是单个下游超时。一个堆转储无法回答这些问题，反而可能造成长暂停和磁盘压力。

```text
先确认时间窗、实例和影响范围
  ├─ CPU 高 → 进程/线程 CPU + 多次线程栈或 JFR
  ├─ 延迟高、CPU 不高 → 线程状态、连接池、下游时间线
  ├─ GC 频繁 → GC 日志中的分配率、存活集和暂停
  └─ 内存持续涨 → 先看堆/非堆趋势与类直方图，再评估 heap dump
```

多次轻量快照比一张孤立快照更能说明变化：同一个线程连续停在同一锁等待点，和线程栈不断前进但整体吞吐低，是两种问题。采集时要同时记录时区、PID、容器实例、JDK 和启动参数，否则事后可能把不同进程或不同发布版本的证据拼在一起。

诊断也有停止条件。若工具会放大故障、磁盘空间不足、转储含大量敏感数据，先恢复服务或复制流量，再在受控环境采集。目标是用最小风险验证下一个假设，不是把所有 JVM 命令执行一遍。
