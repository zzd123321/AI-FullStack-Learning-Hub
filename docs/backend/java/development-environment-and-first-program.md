...
初学者最容易把所有红字都叫作“Java 报错”。实际上，错误发生在哪个阶段，决定了应该检查什么：

```text
javac HelloBackend.java
  ├─ 源码少了分号 → 编译错误：JVM 还没有启动
  └─ 生成 HelloBackend.class
       ↓
java HelloBackend
  ├─ 类名或 classpath 不对 → 启动错误：JVM 找不到入口类
  └─ 找到 main 并开始执行
       ↓
    代码除以 0 → 运行时异常：已经执行到这条语句
```

这条边界很实用：`javac` 报错时先看源码和类型；`Could not find or load main class` 时先看当前目录、类名与类路径；看到异常栈时从第一条属于自己代码的栈帧开始找。不要一看到错误就反复重装 JDK。

与浏览器里的 JavaScript 相比，Java 通常多了一个显式编译阶段。JavaScript 引擎也会解析和编译代码，但前端开发者平时把这些工作交给浏览器或 Vite；Java 的 `javac` 把类型检查结果明确地落成 `.class` 文件，所以“编译成功”与“运行成功”是两件事。
