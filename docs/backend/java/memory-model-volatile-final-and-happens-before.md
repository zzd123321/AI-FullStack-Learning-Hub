...
## 这是一份并发正确性说明书，第一次不必全部读完

并发基础课告诉我们共享状态可能出错，本课进一步回答“根据什么规则证明一个线程一定看得到另一个线程的写入”。这不是在画 CPU 缓存结构，而是在使用 Java 语言规定的可观察行为进行推理。

第一次只掌握三条结论：

1. 没有同步关系时，线程间读取结果不能靠直觉保证。
2. 锁释放到随后获取、写 `volatile` 到随后读，以及任务启动和等待完成，会建立规定的可见性关系。
3. `volatile` 解决不了 `count++` 这种读—改—写复合操作。

CAS、ABA、VarHandle 与内存屏障属于编写底层并发组件时的查阅内容，普通业务优先使用不可变对象、锁和成熟并发工具。

## 把 happens-before 当作一条证据链

假设线程 A 构造一个不可变快照，再把引用写入 `volatile current`；线程 B 随后读取同一个 `current`：

```text
A：写快照字段
  → 程序次序先于
A：volatile 写 current
  → synchronizes-with
B：volatile 读 current
  → 程序次序先于
B：读取快照字段
```

happens-before 具有传递性，因此 B 看到的不只是新引用，也能看到 A 在发布前完成的快照初始化。这是一份由语言规则给出的证明，不是“CPU 缓存大概会及时刷新”的经验判断。

但 `volatile` 修饰的是引用变量 `current`，不会让引用指向的可变对象里每一步复合更新都原子化。若多个线程取得同一个可变列表后分别修改，发布引用本身正确也救不了列表的数据竞争。稳妥模式是先在单线程中构造完整不可变快照，再一次 volatile 写发布新引用。

同理，`volatile int count` 的 `count++` 仍包含读取、加一、写回三步，两个线程可以读取同一个旧值。需要原子计数时使用锁或原子类，而不是把“可见”误解成“整段操作不可分割”。
