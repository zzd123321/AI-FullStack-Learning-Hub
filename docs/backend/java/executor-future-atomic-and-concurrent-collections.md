...
上一课直接创建线程，适合观察底层行为，却不适合让每段业务代码自己决定线程数量和关闭方式。`ExecutorService` 把“要做的任务”与“用哪些线程、允许多少任务、何时关闭”分开。
...
第一次只需要会创建有界执行器、提交任务、处理 `Future` 的成功与失败并正确关闭。CAS、`LongAdder` 和各种并发集合属于共享状态优化，不能替代这条任务生命周期主线。

## 线程池首先是一道准入边界

一个有 4 个工作线程、容量 20 的有界队列，不表示能同时完成 24 个下游调用：4 个任务执行，最多 20 个等待；第 25 个到来时必须由拒绝策略明确决定失败、调用者执行或其他动作。
...
无界队列只是把过载从“明确拒绝”变成“延迟和内存不断增长”。队列容量、任务超时、HTTP 超时和下游承载量应一起设计；线程池不能凭空增加数据库连接或远端服务容量。

`Future` 保存任务的生命周期，不是结果容器的简单包装：任务可能正常完成、抛异常、被取消，或仍未结束。`get(timeout)` 超时只表示调用方等不下去，任务可能继续运行；`cancel(true)` 只是请求中断，任务若不响应中断也未必停止。关闭执行器时应先停止接收新任务，再等待已提交任务，超时后才升级为请求中断。
