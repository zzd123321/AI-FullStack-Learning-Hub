...
请求处理经常把大部分时间花在等待数据库或网络。平台线程在等待期间仍占用较昂贵的操作系统线程资源；虚拟线程让 JVM 可以用少量载体线程承载大量等待中的任务。

它不会让 SQL、HTTP 调用或 CPU 计算本身更快，也不会增加数据库连接数。第一次阅读只需掌握“一个任务一个虚拟线程”和“稀缺下游仍需限流”。`ScopedValue` 与仍处于预览状态的结构化并发属于查阅内容，不应成为学习 Spring Boot 的前置条件。

## 用 10,000 个请求看清瓶颈转移

假设 10,000 个请求大部分时间在等待 HTTP，但数据库连接池只有 20 个连接。虚拟线程可以让这 10,000 个等待任务不各占一个昂贵的平台线程，却不能让数据库同时处理超过 20 个连接。

```text
10,000 个请求任务
  → 10,000 个虚拟线程可以廉价等待
  → Semaphore / 连接池只放 20 个进入数据库
  → 其余任务等待许可，且仍受请求总超时约束
```

因此容量瓶颈往往从“平台线程数”转移到连接池、远端限额、内存或 CPU。若移除原有线程池限制却没有补上针对下游的准入控制，系统可能更快地把压力倾倒给数据库。

虚拟线程适合保持一请求一条顺序调用栈，异常栈和调试体验接近普通阻塞代码。它不是 JavaScript `async/await` 的语法糖：JS await 通常把后续工作交还事件循环，虚拟线程则仍使用线程抽象，只是在阻塞时由 JVM 调度和卸载载体线程。CPU 密集计算不会因为换成虚拟线程而更快，仍应限制并行度接近可用处理器能力。
