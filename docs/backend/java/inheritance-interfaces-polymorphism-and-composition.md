...
上一课把数据和规则装进了对象。如果系统永远只有一种对象，这已经足够。但通知功能今天使用邮件，明天可能增加短信；我们不希望每增加一种渠道，就修改所有调用通知的业务代码。

本课所有术语都围绕同一个问题：**怎样让使用方依赖稳定能力，而把容易变化的实现放到后面？**

```text
NotificationService 只认识 NotificationChannel
                         ↑
              EmailChannel / SmsChannel 提供具体实现
```
