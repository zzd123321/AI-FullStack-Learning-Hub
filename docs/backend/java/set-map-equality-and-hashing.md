...
执行 `map.get(courseId)` 时，可以用下面的因果链理解为什么键必须稳定：

```text
调用 courseId.hashCode()
  → 根据哈希值选择候选位置
  → 在候选键中调用 equals 确认真正相等
  → 返回对应值；找不到则返回 null
```

`hashCode` 不是唯一 ID，不同对象允许产生相同哈希值；冲突时仍要靠 `equals`。反过来，如果两个对象 `equals` 为真却返回不同哈希值，它们可能被放进不同候选位置，集合就无法兑现“相等键视为同一个键”的合同。

最危险的错误是把影响 `equals/hashCode` 的字段在入表后修改。对象最初按旧哈希放入位置 A，修改后查找会按新哈希去位置 B，于是出现“遍历看得见，get 却找不到”的幽灵条目。优先使用 `String`、record 或其他不可变值作为键。

与 JavaScript `Map` 默认按对象身份比较不同，Java `HashMap` 会调用键的 `equals/hashCode`，因此业务值对象可以自然地作为键；代价是你必须正确实现并保持这份相等性合同。
