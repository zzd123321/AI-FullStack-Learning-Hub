...
假设课程代码里到处传递普通字符串：`"BEGINNER"`、`"course-42"`、`"catalog.Course"`。它们看起来简单，却分别隐藏三类错误：类型可能重名、状态可能拼错、不同含义的字符串可能互相传错。

```text
类型重名和归属不清 → package 形成命名空间
状态只能取有限集合 → enum 让非法值无法直接构造
多个值共同定义身份 → record 建立有名字的值类型
```

包名不会因为文件位于某个目录就自动产生；真正的类型名来自源码中的 `package` 声明，目录结构只是让编译工具和人容易找到它。`import` 也不是复制代码，它只让当前文件可以使用简单类名。

记录类很适合课程编号、坐标、只读响应等值对象，但“不可重新给组件赋值”只是**浅不可变**。如果 record 组件是一个可变 `List`，列表内容仍可变化，除非构造时做防御性复制。枚举也不只是常量字符串：每个枚举值都是该枚举类型的唯一实例，因此 `switch` 可以获得编译期的有限集合检查。
