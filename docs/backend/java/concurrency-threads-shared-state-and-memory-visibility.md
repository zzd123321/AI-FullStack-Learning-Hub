...
## 这是一门进阶课，不是 Spring Boot 的入场考试

前面的程序大多只有 `main` 线程，语句看起来按从上到下执行。后端服务器会同时处理多个请求；当两个执行过程读写同一份可变数据时，“每一行单独看都正确”仍可能得到错误结果。
...
先理解“共享可变状态需要协调”、`synchronized` 的基本作用和中断不是强制停止。线程状态、happens-before 细则、死锁诊断与虚拟线程边界可以在项目真正使用并发后再深入。

## start、run 与 join 各自改变什么

```java
Thread worker = new Thread(() -> loadCourse());
worker.start(); // 请求 JVM 安排新线程执行任务；调用方继续向下走
worker.join();  // 当前线程等待 worker 结束，并建立结果可见性
```

若直接调用 `worker.run()`，它只是一通普通方法调用，仍在当前线程执行；不会产生并发。`start()` 返回也不表示任务已开始或完成，两个线程之后的交错次序要由同步关系约束，不能靠“这一行写在前面”猜测。

`join()` 不只是等待。Java 内存模型规定：工作线程结束前的操作 happens-before 另一个线程从成功的 `join()` 返回，因此调用方随后能看到已发布结果。若用 `sleep(100)` 猜任务大概完成，不但时间不可靠，也没有表达正确的生命周期关系。

JavaScript 前端通常由事件循环在一个主线程上调度回调；Promise 表达异步完成，但不自动让普通 JS 代码并行。Java 后端中多个线程可以同时执行 Java 语句，因此共享对象确实可能在同一时段被读写。迁移思维时，不能把 `await` 前后的单线程直觉直接套在 Java 共享状态上。
