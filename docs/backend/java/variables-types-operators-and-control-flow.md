...
先用一个极小的 `StudyGoal` 思维示例说明数据流：假设命令是 `java StudyGoal 90 3`。程序看到的不是两个数字，而是两个字符串（本课后半的完整示例会再加入“星期”这个业务输入）：

```text
args[0] = "90"，args[1] = "3"
  → Integer.parseInt 把文本解析为 int
  → 校验 90 和 3 是否落在业务允许范围
  → 计算每天 90 / 3 = 30 分钟
  → if 根据 30 选择提示语
```

这里每一步都可能改变控制流：参数缺失不能读取索引，文本不是整数会解析失败，负数虽然能解析却不符合业务规则。**类型正确不等于业务有效**，因此“解析”和“校验”应是两个清楚的步骤。

还要特别警惕 Java 与 JavaScript 的数值差异：Java 的 `int` 是 32 位有符号整数，`5 / 2` 得到 `2`，越界还可能回绕；JavaScript 常用的 `Number` 是双精度浮点数，`5 / 2` 得到 `2.5`。把前端算法直接翻译成 Java 时，先确定数值类型和除法语义，再写条件。

```java
int totalMinutes = 5;
int days = 2;
double average = (double) totalMinutes / days; // 先转成 double，结果才是 2.5
```
