...
## Future 能交结果，但不擅长描述“下一步”

普通 `Future` 让调用方拿到未来结果，但组合多个依赖任务时，代码容易变成连续 `get()`。`CompletableFuture` 的核心不是“更快”，而是把任务之间的先后、并行和失败关系写成一张完成图。
...
第一次学习先区分 `thenApply`（同步转换已有结果）、`thenCompose`（下一步也返回异步任务）和 `thenCombine`（合并两个独立结果）。超时不等于停止底层任务、取消不一定中断线程，是第二遍必须补上的工程边界。

## 先画依赖关系，再选择方法名

假设生成学习首页需要“先查用户，再按用户查统计”，同时“推荐课程”与统计互不依赖：
...
“查统计”依赖用户结果且自身返回 future，所以用 `thenCompose` 展平；“统计”和“推荐”彼此独立，所以先分别启动，再用 `thenCombine` 汇合。若只是把已经完成的用户对象转换成显示名，才使用 `thenApply`。方法名是依赖图的结果，不应靠试到编译通过。

还必须标出每个阶段在哪里执行。没有 `Async` 后缀的阶段，可能由完成上一阶段的线程执行，也可能在注册时因前序已完成而由当前线程执行；带 `Async` 却不传 executor，则通常进入公共池。后端代码应为阻塞 IO 使用受控执行器，避免一个慢依赖占满全进程共享资源。

```text
orTimeout 到期 → CompletableFuture 以超时异常完成
               ↘ 底层 HTTP/文件调用可能仍继续
```

所以完整超时设计还要把期限传到真正的 HTTP 客户端或任务，并考虑晚到结果如何丢弃。JavaScript 的 Promise 也不会因外层超时自动取消底层 fetch；二者都需要显式取消协议和资源所有权。
