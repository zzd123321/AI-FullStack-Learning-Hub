...
## 只有普通 synchronized 不够时，才进入本课

显式锁和同步器不是 `synchronized` 的“高级替代品”。它们分别解决可中断获取、限时等待、多个条件队列、并发许可和阶段会合等更具体的问题，也引入忘记释放锁或许可的新风险。

第一次阅读只建立一张问题地图：互斥用锁，等待状态变化用 `Condition`，限制同时访问下游的数量用 `Semaphore`，等待多个事件完成用 `CountDownLatch`。如果当前项目没有这些问题，知道回来查即可。

## 用容量为 1 的队列看懂 Condition

生产者发现队列已满时，不能拿着锁不断循环，否则消费者永远拿不到锁来移走元素。`notFull.await()` 会原子地完成三件事：把当前线程放进条件等待队列、释放关联锁、暂停；消费者移走元素并 `signal` 后，生产者还要重新竞争并获取锁，才能从 `await` 返回。

```text
生产者持锁，发现 full
  → await：登记等待 + 释放锁
  → 消费者获锁，取走元素，signal，释放锁
  → 生产者重新获锁
  → 再次检查 full，确认条件后写入
```

必须用 `while` 重查条件，因为唤醒只表示“现在值得再检查”，不保证条件仍成立：别的生产者可能先一步占用空间，也可能发生虚假唤醒。`Condition` 管的是等待某个状态，`Lock` 管的是同一时刻谁能读写这份状态，两者不能互相替代。

`Semaphore(20)` 则不保护某个对象不变量，它表示最多 20 份许可，可用来约束同时访问下游的任务数。只有成功 `acquire` 后才应在 `finally` 中 `release`；否则中断发生在获取之前却错误归还许可，会让许可凭空增加。
