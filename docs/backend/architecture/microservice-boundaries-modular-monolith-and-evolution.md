...
它们在一个进程中部署，不代表必须互相随意访问。模块化单体可以规定：

```text
catalog 只通过 CatalogQuery 暴露课程信息
enrollment 拥有报名规则和报名数据
progress 只通过 enrollment 提供的资格接口判断能否记录进度
notification 订阅已发生的业务事件，不直接改其他模块的表
```

这时本地方法调用、统一测试和一次部署都很简单，同时逻辑所有权已经清楚。这个状态不是“还没做架构”，而是在低运维成本下验证边界。
...
- 邮件和短信有突发流量，需要独立扩缩容；
- 第三方供应商经常变慢，不应占满核心请求线程；
- 通知团队需要独立发布模板和供应商适配；
- 核心报名流程只要求可靠记录通知任务，不要求当场发送完成。

这是一组可验证的拆分理由。相比之下，“notification 代码超过 5000 行”并不能单独证明它需要成为远程服务。

拆分前先在单体内建立 port：

```java
public interface NotificationPort {
    void requestEnrollmentNotice(EnrollmentNotice notice);
}
```

第一阶段它指向本地实现；边界稳定后再换成消息或远程 adapter。业务调用者依赖的是“请求通知”合同，不依赖进程位置。
...
JSON 字段不兼容 → 部署后才暴露
服务地址错误 → 连接失败
对方变慢 → timeout、排队和资源耗尽
响应丢失 → 结果未知，可能需要幂等重试
两边发布不同步 → 需要向后兼容合同
```

因此“换成 HTTP”不是实现细节，而是引入了一整套失败语义。拆分收益必须大到值得承担这些成本。

## 用三个问题检查真假边界

### 1. 谁能修改这份数据

真正的数据所有权意味着其他模块/服务不能绕过 owner 直接写表。如果 Ordering 和 Inventory 都直接更新同一库存表，就没有独立不变量和发布边界。

### 2. 一项需求会改动几个边界

如果“订单增加备注”每次都要同时修改五个服务、五条流水线和五个版本，边界没有减少共同变化，只是把它分散了。

### 3. 一个边界能否独立失败和恢复

如果通知服务宕机必然让报名 API 同步失败，或者所有服务共享一个连接池和数据库锁，它们没有得到预期的故障隔离。
...
注意最后一步通常最难。代码已经走远程 API，但两个服务仍共享数据库，只是“接口看起来像微服务”；真正迁移所有权还需要同步、校验、回滚和停止旧写入的计划。
