...
不要从“系统要 99.99% 高可用”开始。先选一个用户真正关心的动作，例如“打开课程详情”：

```text
有效请求：已认证用户请求一个存在且有权查看的课程
成功：2 秒内返回正确课程内容
统计窗口：滚动 28 天
目标：99.9% 的有效请求成功
```
...
- **SLI**：有效请求中，2 秒内返回正确结果的比例；
- **SLO**：28 天窗口内至少达到 99.9%；
- **bad event**：超时、5xx 或内容错误；
- **不计入项**：无权限、客户端取消等是否排除必须明确约定。

只写“API 可用性”不够，因为 `/health` 返回 200 时，课程详情仍可能全部超时。
...
```text
平日峰值：200 request/s
活动预计峰值：500 request/s
平均服务时间：80 ms = 0.08 s
单实例达到 60% CPU 时可稳定处理约 100 request/s
任何一个 zone 故障后仍要承载活动峰值
```

Little's Law 先给出活动峰值的平均在途请求：

```text
L = λ × W = 500/s × 0.08s = 40 个并发请求
```

再用实测单实例安全吞吐估算：

```text
正常承载 500/s 至少需要 5 个实例
但若实例分布在 3 个 zone，失去 1 个 zone 后仍要承载 500/s
因此不能只按正常时刚好 5 个部署
```

例如每个 zone 3 个实例，共 9 个；失去一个 zone 后剩 6 个，理论安全吞吐约 600/s。这个数字仍需负载测试验证数据库、缓存和第三方依赖是否也有相同余量。
...
“CPU 到 80% 自动扩容”没有回答新实例 ready 前谁承受 burst，也没有回答数据库是否会先达到连接上限。

## 用错误预算理解 99.9%，不要只背几个 9

28 天有 40,320 分钟。若粗略按时间可用性换算：
...
```text
允许 bad events = 10,000,000 × 0.001 = 10,000 次
```

两种算法不是自动等价：一次 5 分钟全站故障和一个月内持续 0.02% 的零散错误，对用户和告警策略影响不同。SLO 必须说明按请求、时间还是业务任务计算。

错误预算的用途不是鼓励“把 10,000 次错误用完”，而是把可靠性讨论变成决策：

- 预算充足时，可以正常发布并改善功能；
- 预算快速燃烧时，停止高风险变更并优先修复；
- 某个租户或地区单独恶化时，不能被全局平均值掩盖。

## 高可用和灾难恢复解决不同时间尺度

```text
一个实例崩溃
  → 负载均衡从 ready endpoints 中移除
  → 其他实例继续服务
  → 属于高可用与故障转移
...
replica 会快速复制好数据，也可能快速复制误删除或逻辑损坏；它不等于独立备份。备份文件存在也不等于能够在目标时间内恢复，只有演练过“选择恢复点 → restore → 校验 → 切流”的完整过程才是证据。

第一次阅读本课，先完成“一个用户旅程的 SLI/SLO + 峰值容量 + 单故障余量”。RPO、PITR、多区域和 failback 应在业务影响分析明确后再继续。
