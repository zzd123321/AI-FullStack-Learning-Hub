...
这个注解告诉开发者和静态检查器：调用点应提供 `str`，函数正常返回 `Task`。但运行时直接调用 `complete_task(123)`，CPython 通常不会因为注解自动拒绝。

来自 HTTP 的 JSON 还需要运行时验证：

```text
外部 JSON（完全不可信）
  → Pydantic/显式 parser 验证并转换
  → 得到内部 TaskCommand
  → 类型检查器分析内部调用关系
  → 测试执行成功、失败和副作用路径
```

类型提示不是验证库，验证库也不能证明所有内部调用在静态上兼容。
...
`object` 表示值未知，使用前必须收窄；`Any` 会让检查器放弃很多检查，错误继续传播。系统边界可以暂时接收 `object`/未知结构，经过验证后尽快转换成精确类型，不应让 `Any` 穿过所有业务层。

## 测试证明行为，不只证明能运行

```text
静态检查：不存在把 Optional[Task] 当 Task 直接使用的路径
单元测试：任务从 pending 完成后状态和时间正确
失败测试：已完成任务再次完成会抛稳定异常
集成测试：Repository adapter 遵守同一 Protocol/合同
```

追求 100% coverage 不能替代有意义断言；只有类型没有测试，也无法发现金额公式写错或事务副作用。第一次先给公共函数写准确输入/输出/None 类型，并为一条成功和一条失败路径写测试。
...
第一次学习只需做到：给公共函数写清输入和返回类型，正确表达 `None`，并能运行测试。泛型、Protocol 和复杂类型收窄是接口开始复用后再引入的工具，不要为了追求类型技巧让简单业务难以阅读。
