...
只用 `new` 创建对象时，创建者必须同时知道具体实现、构造参数、共享方式、初始化顺序和关闭时机。小程序可以这样工作，但后端应用很快形成一张对象图：Controller 依赖 Service，Service 依赖 Repository，Repository 又依赖数据源、事务和监控基础设施。

如果每一层都自行创建下一层，会产生一条难以替换的因果链：
...
Spring 容器把“对象如何定义、何时创建、依赖谁、共享多久、是否包装代理、何时销毁”提升为集中管理的运行时模型。业务对象仍然是普通 Java 对象；容器的价值是管理那些构成应用基础设施和长期协作图的对象，而不是取代所有 `new`。

理解生命周期和 scope 也直接关系到正确性：默认 singleton 会被并发请求共享；request 对象只能活在请求范围；代理可能改变你真正调用的对象；循环依赖则说明对象图无法按构造顺序建立。
...
下一节：[Spring Boot MVC 参数绑定、输入校验、统一错误响应与测试](/backend/spring-boot/mvc-parameter-binding-validation-error-response-and-testing)。
