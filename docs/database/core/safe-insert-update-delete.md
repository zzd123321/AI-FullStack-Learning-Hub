...
::: tip 第一次学习只抓住四件事
- **必须理解**：写操作同样必须参数绑定；`UPDATE`、`DELETE` 必须有经过确认的条件；数据库约束是并发下的最终防线。
- **必须会用**：检查 affected rows、取得生成 ID，并把“未找到”和“更新成功”区分开。
- **必须完成**：实现一次新增和一次带主键条件的更新，重复请求不会制造不可接受的副作用。
- **可以后看**：批量写入、各种 UPSERT 方言和复杂幂等状态机。
:::
