...
::: tip 第一次学习只抓住四件事
- **必须理解**：ORM 最终仍生成 SQL；实体对象、事务和连接都有生命周期，Repository 负责隔离数据访问细节而不是隐藏数据库事实。
- **必须检查**：生成 SQL、参数绑定、类型映射、查询次数以及事务中的所有操作是否使用同一连接。
- **必须完成**：实现一个 Repository 方法，并通过日志确认没有 N+1 且事务边界正确。
- **可以后看**：缓存、批处理、复杂关联映射和框架专属生命周期细节。
:::
