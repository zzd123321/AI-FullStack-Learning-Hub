...
::: tip 第一次学习只抓住三件事
- **必须理解**：应用连接数据库需要地址、端口、数据库名、用户名和凭据；连接成功不代表拥有任意权限。
- **必须完成**：选择项目实际使用的一种数据库，能连接学习库并执行一条只读查询。
- **可以后看**：另一种数据库的全部命令和管理细节。先记住关键差异存在，需要时再查。
:::
...
完成本课后，你应能画出客户端、数据库服务器与一次会话的关系，确认当前连接的身份和默认命名空间，并解释 MySQL 的 database 与 PostgreSQL 的 database/schema 为什么不能直接等同。它承接第一条 SQL 的关系模型，并为下一课把接口字段落实为列类型、默认值和约束做好环境基础。
