...
::: tip 第一次学习只抓住四件事
- **必须理解**：一对一、一对多、多对多描述的是业务基数；外键列通常放在“多”的一侧，多对多需要关联表。
- **必须会写**：用 `INNER JOIN` 取必然存在的关联，用 `LEFT JOIN` 保留左表记录，并把连接条件写在 `ON` 中。
- **必须完成**：把一个循环逐条查库的简单 N+1 查询改成一次合理 JOIN。
- **可以后看**：复杂自连接、横向连接和大型结果集装配策略。
:::
