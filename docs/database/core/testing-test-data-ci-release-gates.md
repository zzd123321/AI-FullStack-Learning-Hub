...
::: tip 第一次学习只抓住四件事
- **必须理解**：mock 只能验证调用方式，不能证明 SQL、约束、类型映射、migration 和事务在真实数据库中正确。
- **必须覆盖**：Repository 的成功与失败路径、关键约束、事务回滚以及从空库执行 migration。
- **必须完成**：使用项目实际数据库运行一组可重复、相互隔离的集成测试。
- **可以后看**：大规模测试数据生成、生产流量回放和复杂 CI 发布门禁。
:::
