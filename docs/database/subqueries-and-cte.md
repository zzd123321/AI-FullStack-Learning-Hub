...
::: tip 第一次学习只抓住三件事
- **必须理解**：子查询和 CTE 是表达查询步骤的工具，不会自动让 SQL 更快；正确性和执行计划仍需验证。
- **必须完成**：能用一个非递归 CTE 把“先筛选或汇总，再连接”的查询拆成可单独检查的步骤。
- **可以后看**：相关子查询优化、递归 CTE、物化行为和数据库版本差异。
:::
...
完成本课后，你应能先判断中间查询应该产生一个值、一列值还是一张关系，再选择标量子查询、`EXISTS`、派生表或 CTE；还能解释 `NOT IN` 与 `NULL` 的风险。它承接 JOIN 与聚合中的“结果粒度”思想，并为下一课的安全写入和之后的执行计划分析提供可读的查询拆分方法。
