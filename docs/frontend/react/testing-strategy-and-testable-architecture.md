...
测试架构的核心不是 Mock 更多依赖，而是把系统拆成可独立证明的风险边界：纯领域规则用快速单测，组件通过可访问 DOM 验证用户行为，HTTP/Router 用真实协议形状做集成，少量 E2E 证明浏览器、服务端和持久化共同工作。

等待机制必须对应真实异步来源，Mock 必须位于测试目标之外，Flaky 必须作为产品或测试设计缺陷处理。Coverage 只能显示哪些代码运行过，不能证明关键状态转换、竞态、权限和恢复路径正确。
