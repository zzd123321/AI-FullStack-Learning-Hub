...
设想你正在维护一个 Vue 2 学习平台。课程列表、详情、报名和权限已经缠在全局 Store、Event Bus 与请求拦截器里；团队希望逐步引入 React，却不能暂停业务半年重写。此时最危险的做法，是先讨论微前端和目录模板。它们只会把原有耦合搬到更多仓库和运行时里。
...
这条顺序很重要：先让模块能够独立，再让它独立部署。否则“微前端”只是把编译期错误变成线上网络错误。
...
这一阶段不需要更换框架，也不需要拆仓库。目标是让“课程”这个业务能力知道自己依赖什么，并阻止其他功能穿过公开入口读取它的内部实现。
...
## 让 Vue 2 与 React 在迁移期安全共存

前面的端口、DTO 和状态所有权稳定后，React 才有一块可以接管的业务切片。共存层的职责很窄：管理生命周期、翻译契约、隔离故障，并且最终能够删除。它不应成为新的永久业务层。
...
### Vue 2 宿主挂载 React：显式 Imperative API
...
### Custom Element：更标准的框架边界

Vue 2 宿主可以消费原生 Custom Element；为了避免把自定义标签误当作 Vue 组件，需要按 Vue 2 工程配置 `Vue.config.ignoredElements`。Vue 3 则提供更完整的消费能力，并可用 `defineCustomElement` 创建元素。React Widget 同样可以包装成 Custom Element：
...
- 新平台还提供配合 `Element.moveBefore()` 的 `connectedMoveCallback()`，但采用前必须核对目标浏览器；普通 `insertBefore()` 等移动仍需正确处理 Disconnect/Reconnect。
...
到这里，即使所有模块仍在一个仓库和一条流水线中，迁移也已经可以持续进行。只有当团队边界和发布节奏确实无法共享部署单元时，才需要承担运行时组合的额外成本。
...
无论是单体、双框架还是微前端，生产风险最终都落到同一组问题：新版本先给谁、出现什么信号时暂停、旧版本能否读取新数据、回滚是否真的有效。发布能力属于架构本身，不是流水线末尾的附属配置。

### Feature Flag 是发布控制，不是权限系统
...
### 把 Feature Flag 当永久架构

新旧代码永远共存，测试组合持续增长。Flag 创建时就创建删除任务和截止日期。

### 只测最新版 Host + 最新版 Remote
...
- [Vue 2：`ignoredElements`](https://v2.vuejs.org/v2/api/#ignoredElements)
...
下一节进入“浏览器与网络”模块，从[事件循环、任务与微任务、渲染流水线和长任务诊断](../browser/event-loop-rendering-and-long-tasks.md)开始，把框架层现象还原为浏览器执行模型。
