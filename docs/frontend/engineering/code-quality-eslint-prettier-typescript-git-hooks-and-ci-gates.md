...
```ts
async function submitLesson(id: string): Promise<void> {
  const lesson = await loadLesson(id)
  renderLesson(lesson)
}

submitLesson(route.params.id)
```

它从保存到合并会依次经过不同视角：Prettier 只决定怎样排版；ESLint 可以发现最后一行的 Promise 没有被处理；TypeScript 检查 `route.params.id` 与函数参数是否兼容；测试验证失败提示和渲染行为；构建确认真实模块与资源能够生成；CI 则保证所有人面对的是同一把尺子。

这就是本课的主线：工具不是越多越好，而是让每一道门只回答一个清楚的问题，并把昂贵检查放到合适的位置。
...
## 静态分析：ESLint 先要知道文件运行在哪里

### Flat Config 是有顺序的配置数组
...
## TypeScript 配置也在描述运行时边界

### 不要让一份 tsconfig 同时假装运行在浏览器和 Node.js
...
### 所有目录共享 browser、Node 和 test globals
...
下一节：[前端测试：单元、组件、E2E 与可靠性](./frontend-testing-unit-component-e2e-and-reliability.md)
