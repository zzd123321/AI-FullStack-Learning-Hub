...
#### Processing Duration：Handler 实际做了多少同步工作
...
#### Presentation Delay：Handler 返回后为什么还没画出来
...
### RUM：安全采集真实 Core Web Vitals

#### 为什么优先使用官方 `web-vitals` 库
...
- 只保留本次访问最慢的五个样本；
- 同时上报缓冲区丢失的 Entry 数，避免把不完整样本当成完整事实；
- 页面隐藏时上报并清空；
...
### Navigation、Resource 与 Server Timing
...
示例为并发操作生成唯一 Mark，避免两次同名操作互相覆盖；在 `finally` 中完成 Measure 并清理 Mark，Observer 消费 Measure 后也会将它从性能时间线移除，避免长会话无限积累。
...
#### 只看一次 Lighthouse 100 分

无法代表真实设备、交互和长会话，也没有版本趋势。应结合 RUM p75、Trace 与持续预算。

#### 给所有资源 preload / high priority
...
下一节：[大型前端架构：模块边界、领域分层与渐进演进](../architecture/large-scale-frontend-modules-boundaries-and-micro-frontends.md)
