...
- 区分 origin、site、same-origin 与 same-site；
- 理解同源策略限制的是哪些“读、写、嵌入”，以及它不保护什么；
- 从数据源到执行 sink 分析反射型、存储型和 DOM 型 XSS；
- 正确选择文本编码、HTML 清洗、CSP 和 Trusted Types，而不是混为一谈；
- 解释 Cookie 认证为什么会产生 CSRF，并组合 token、Origin、Fetch Metadata 与 SameSite；
- 正确配置带凭证的 CORS 和预检响应；
- 安全使用 `postMessage`、iframe、外部链接及资源加载；
- 理解 COOP、COEP、CORP 与 `crossOriginIsolated` 的分工和部署代价；
- 为安全策略建立灰度、报告、自动测试和生产观测机制。
...
## 4. XSS 的本质：不可信数据获得了当前 origin 的代码权限
...
## 5. 第一原则：如果只需要文本，就永远不要进入 HTML 解析器
...
## 8. Trusted Types：强制危险 sink 经过统一策略
...
### 11.2 Origin、Fetch Metadata 与 token 一起校验
...
## 12. CORS：服务器授权浏览器中的某些 origin 读取响应
...
## 13. `postMessage`：跨 origin 窗口通信需要完整协议
...
下一节将进入前端工程化模块，从 Vite 的开发服务器、依赖预构建、原生 ESM、插件流水线与生产构建边界开始。
