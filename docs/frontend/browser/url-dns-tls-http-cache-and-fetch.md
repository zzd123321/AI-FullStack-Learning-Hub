...
- 正确拆解 URL，区分 URL、Origin、Site 与 Resource Identity。
- 解释 DNS 的递归解析、缓存、A/AAAA、CNAME 和 HTTPS/SVCB 记录在链路中的作用。
- 区分 HTTP/1.1、HTTP/2 与 HTTP/3 的传输、复用和队头阻塞特征。
- 理解 TLS 1.3 的证书验证、SNI、ALPN、会话恢复与 0-RTT 风险。
- 从 `fetch()` 调用追踪到 Service Worker、HTTP Cache、CORS、连接和 Response Body Stream。
- 正确处理 `fetch` 的 Network Error、HTTP Error、超时、取消和 Runtime Schema Validation。
- 区分 `no-cache`、`no-store`、`private`、`immutable`、`max-age` 和 `s-maxage`。
- 使用 ETag/If-None-Match 与 304，理解 Cache Key 和 `Vary`。
- 为重试定义方法安全性、幂等性、Backoff、Jitter、Retry-After 和取消语义。
- 设计严格 Origin Allowlist、Credentialed CORS 和 Preflight Handler。
- 判断 HTTP Cache、Service Worker Cache、应用内 Query Cache 与 BFCache 的边界。
- 使用 Resource Timing 和 Network 面板诊断 DNS、连接、TTFB、下载、协议与缓存。
...
## 10. HTTP/3：HTTP Semantics over QUIC
...
## 20. Credentials、Cookie 与 SameSite
...
`If-None-Match` 对 GET/HEAD 使用 Weak Comparison，所以示例规范化 `W/`。真实 ETag 生成必须稳定且符合 Representation：若 Gzip/Language 版本内容不同，要配合不同 ETag 或正确 `Vary`。

## 24. Strong/Weak Validator 与并发控制
...
## 26. Fetch Cache Mode 不是 Response Cache-Control
...
## 27. Cache API 与 HTTP Cache 是两套系统
...
## 28. 一个克制的 Service Worker 策略

<<< ../../../examples/frontend/browser-network-fetch/service-worker.ts

示例只做两件事：

- Navigation 使用 Network-first，失败时回退已缓存页面或 Offline Page。
...
下一节继续浏览器模块，深入 DOM、事件传播、默认行为、事件委托、Shadow DOM、Pointer/Input 事件与可访问交互模型。
