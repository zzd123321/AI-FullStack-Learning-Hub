...
示例处理 Primary Button、Capture、Coalesced Move 的兼容检测、Pointer Up/Cancel 和清理，并在拖动期间设置 `touch-action: none`。视觉更新用 `transform`，高频产品可把最新坐标存起来并在 rAF 中每帧更新一次。不要只监听 Mouse，否则 Touch/Pen 失效；也不要在非拖动区域阻止所有 Touch 手势，破坏页面滚动。
