...
- 区分 ECMAScript Job、HTML Task、Microtask 和 Rendering Opportunity。
- 理解 Run-to-completion 为什么既降低并发推理难度，也会造成页面卡顿。
- 推导同步代码、Promise、`queueMicrotask`、Timer 和 `requestAnimationFrame` 的常见执行关系。
- 解释为什么 `await Promise.resolve()` 不能把渲染机会还给浏览器。
- 正确使用 Microtask 做同一轮批处理，并避免 Microtask Starvation。
- 理解 `requestAnimationFrame` 不是固定 16.7ms Timer，并写出刷新率无关动画。
- 区分 Style、Layout、Paint、Raster 与 Composite，识别 Forced Synchronous Layout。
- 用读写分离避免 Layout Thrashing。
- 使用 `scheduler.yield()` 和兼容回退拆分可中断的 CPU 工作。
- 判断何时应把计算移到 Web Worker，并设计可校验、可取消的消息协议。
- 使用 Performance 面板、Long Tasks、Long Animation Frames 和 Event Timing 定位交互延迟。
- 把事件循环知识映射到 Vue、React、网络回调和生产性能治理中。
...
## 3. Run-to-completion：任务不会被 JavaScript 抢占
...
## 7. `Promise.then`、`queueMicrotask` 与 `async/await`
...
## 10. Rendering Opportunity 不是每个 Task 后必然 Paint
...
## 11. `requestAnimationFrame` 的正确心智模型
...
## 18. 使用 `scheduler.yield()` 主动让出主线程

`scheduler.yield()` 专为把 Continuation 放到未来 Task，并尽量保持 Continuation 优先级而设计。由于支持范围会变化，代码应 Feature Detect 并提供回退：
...
## 24. 使用 PerformanceObserver 采集线索
...
- **Long Animation Frame（LoAF）**：一帧为何过长，并可在支持实现中提供 Script Attribution。
...
## 26. Chrome DevTools Performance 诊断流程
...
下一节将继续浏览器与网络模块，系统学习 URL、Origin、DNS、TCP/TLS、HTTP 缓存、连接复用，以及一次 `fetch()` 从发起到响应可用的完整路径。
