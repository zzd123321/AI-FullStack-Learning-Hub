...
假设用户在断网时编辑了一篇长草稿。页面显示“已保存”，随后另一个标签页也修改了它；网络恢复后两个标签页同时上传，其中一个请求其实成功了，但响应丢失。这个场景已经不是“把 JSON 存进浏览器”，而是本地事务、多写者协调、网络不确定性和远端冲突共同组成的一致性问题。

本课按难度递进：先为数据选择正确生命周期，再掌握 IndexedDB 的事务与迁移，最后才搭建带 Outbox、租约、幂等和冲突检测的离线同步链路。
...
同样重要的是完成阶段：旧执行者收到网络响应时，租约可能已经过期并被新标签页认领。因此 `complete`、`reschedule` 和 `fail` 都必须在新事务中重新读取记录，只有 `leaseOwner + attempts` 仍匹配时才能条件更新；否则旧执行者放弃写入，由当前 Owner 继续处理。

这条条件被提取为可单测的纯策略：

<<< ../../../examples/frontend/browser-storage/lease-policy.ts
...
### Cache API：显式的 Request / Response 仓库
...
下一节进入[浏览器安全模型：XSS、CSRF、CSP 与跨源隔离](./browser-security-xss-csrf-csp-and-cross-origin-isolation.md)，分析本地数据、身份凭证和跨窗口消息应怎样受到真正的安全边界保护。
