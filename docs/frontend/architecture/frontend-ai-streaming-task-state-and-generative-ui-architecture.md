...
这些问题看似分散，其实来自同一个误解：把一次 AI 生成当成了普通 HTTP 请求。更合适的心智模型是“一个会持续产生事件、可能暂停等待外部动作、也可能被恢复的异步任务”。

本课沿着一次回答真实经过的路径展开：先划清浏览器与后端的信任边界，再把字节流还原成语义事件；随后用请求身份和状态机管理它，最后才加入工具、生成式 UI、安全和生产治理。示例以 OpenAI Responses API 说明供应商适配，但组件和浏览器协议不依赖具体 SDK。
...
动手写聊天气泡之前，先回答两个问题：秘密放在哪里，谁有权执行动作。只要这两个边界模糊，后面的状态管理写得再漂亮也无法补救密钥泄露或越权工具调用。
...
自有后端中的 adapter 把供应商事件转换为应用内部小协议：

<<< ../../../examples/frontend/ai-streaming-ui/provider-event-adapter.ts

这里特意把 adapter 放在独立文件中，是为了说明它属于供应商边界；生产项目通常让它运行在 BFF，而不是把供应商原始协议透传到浏览器。它有三个价值：

1. 供应商升级时只修改一个边界；
2. reducer 测试不需要构造庞大的供应商对象；
3. 产品可以统一多供应商或 Mock 的事件语义。

工具调用的时序尤其容易写错：`response.output_item.added` 只表示函数调用项已经出现，参数可能还在通过 `response.function_call_arguments.delta` 继续到达；示例直到 `response.output_item.done` 才产生可进入后续校验的 `tool-call`。过早展示审批会让用户看到、甚至批准一组尚未完成的参数。
...
parser 还设置了未完成 frame 的容量上限。没有这个边界时，对端只要持续发送不含分隔符的数据，前端 buffer 就会无限增长。这个上限约束的是已解码字符数量；若协议按字节计费或限流，服务端还需要单独限制原始字节。

若自有后端使用 NDJSON、自定义 length-prefix 或 WebSocket，替换 parser 即可；任务 reducer 不应改变。这就是把传输格式与产品状态拆开的直接收益。

### 请求客户端：取消、错误和资源所有权

浏览器不再解析 OpenAI 事件，而只消费自有后端发出的 `GenerationEvent`。这个小协议同样需要运行时校验，因为 TypeScript 类型不能验证网络数据：

<<< ../../../examples/frontend/ai-streaming-ui/generation-events.ts
...
传输层现在能可靠地产生事件，但事件仍不能直接改 DOM。用户可能取消 A 后立刻提交 B，工具调用会让生成暂停，组件也可能在网络结束前卸载。下一步是把所有变化收口为一个有身份、有合法阶段的任务。
...
Responses API 支持使用 `previous_response_id` 延续上下文，也可以由应用手动管理输入。按当前官方说明，Response 对象默认保存 30 天，可通过 `store: false` 关闭；Conversation 中的 items 不受这项 30 天 TTL 限制。使用 `previous_response_id` 时，链上此前输入 token 仍会作为输入计费。前端不应据此把供应商 ID 当作自己的会话主键或长期数据库。
...
到这里，纯文本回答已经能够可靠开始、流动、停止和完成。工具调用与生成式 UI 不是另一套系统，而是在同一任务中加入新的 part 和暂停点；它们同时把“模型说了什么”升级为“应用可能做什么”，因此信任边界必须更严格。
...
示例按每种 block 检查字段类型、字符串长度、数组大小与表格宽度；真实项目还应拒绝额外属性、验证引用 ID 的访问权限，并为 schema 与历史数据设置版本。
...
在 Responses API 中，面向用户的结构化输出通过 `text.format` 配置 JSON Schema。成功且未中断的 strict 输出可以遵守给定 schema，但调用方仍必须单独处理拒绝、达到输出上限等未完整生成的结果。即使形状完全正确，也不能证明内容真实、引用有权限或动作安全；Schema validation 之后仍要做领域校验。
...
可靠和安全只是底线。回答持续几十秒时，滚动、读屏、错误恢复与再次输入会共同决定用户能否理解系统正在做什么。下面这些不是“最后再补的体验细节”，而是任务语义在界面上的表达。
...
单次演示成功不等于可以上线。生产系统还要回答：高频增量是否卡住主线程、长会话成本如何增长、刷新后怎样恢复、什么数据可以记录，以及协议升级后哪些测试会先报警。
...
下面的契约测试还专门证明：工具项刚出现时不会进入审批，只有完整 item 才会转为应用事件；浏览器协议也会拒绝错误时间戳和其他请求的迟到事件。

<<< ../../../examples/frontend/ai-streaming-ui/provider-event-adapter.test.mts
...
如果现有项目仍是一次性返回完整字符串，不必同时引入工具、生成式 UI 和复杂恢复。按风险从低到高分层落地，每一层都应先做到可停止、可观测、可安全失败。
...
#### 在浏览器调用供应商并打包 API Key

密钥对用户可见，权限、成本和安全策略也无法可靠执行。请求必须经过自有后端。

#### 按网络 chunk `JSON.parse`
...
下一节：[前端语音、音频与实时多模态交互架构](./frontend-realtime-voice-audio-and-multimodal-interaction-architecture.md)，会在本课任务状态与信任边界之上，继续处理麦克风权限、WebRTC、音频播放、打断与实时会话。
