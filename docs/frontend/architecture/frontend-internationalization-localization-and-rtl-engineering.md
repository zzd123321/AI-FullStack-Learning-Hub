...
## 一、Internationalization 与 Localization
...
## 二、Locale 不是 Language，也不是 Country
...
## 十三、List、Relative Time、Display Name 和 Collation
...
对于方向未知的用户生成文本，`<bdi>` 可以隔离其方向，避免订单号、用户名或标题改变周围标点顺序。

## 十六、RTL 布局使用 Logical Properties
...
### 29.3 Locale 等于 Country

错误推断语言、Currency 和 Time Zone。它们是相关但独立的上下文。

### 29.4 用用户 Locale 推断 Time Zone
...
下一节建议进入**前端数据可视化与复杂交互架构**，讨论 Canvas/SVG/WebGL 取舍、坐标与 Scale、交互状态、可访问替代、流式数据、性能和图表组件治理。
