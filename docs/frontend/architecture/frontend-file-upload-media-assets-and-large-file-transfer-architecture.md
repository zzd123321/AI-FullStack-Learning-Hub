...
如果进度条到 100% 就显示“发布成功”，后续扫描或转码失败会让 UI、业务数据库与对象存储互相矛盾。问题的根源是把“字节已经发完”和“业务资产已经可用”当成了同一件事。

本课沿着一份文件的真实生命周期展开：先理解浏览器拿到的 `File`，再建立由后端控制的上传协议；随后处理分片、进度、重试和恢复，最后进入扫描、转码、发布与清理。每一步都明确谁拥有事实，避免只罗列 API。
...
用户选择文件只是授权页面读取一组字节，并没有证明格式、安全性或业务权限。先把选择、预览和即时校验放回正确位置，后面的上传协议才不会建立在伪造元数据之上。
...
这个检测器只能识别少数容器签名，不能证明文件安全，也不能完整识别复杂格式。它把带 `ftyp` box 的内容识别为更宽泛的 `iso-bmff`，而不是直接断言为 MP4，因为 HEIF、QuickTime 等也属于这一容器家族。例如：
...
浏览器知道文件字节，后端才知道用户权限、存储供应商限制和资产归属。因此分片大小、对象键、短期权限与完成条件都应由上传会话确定，而不是散落在组件常量里。
...
协议确定后，前端面对的核心不再是“发一个请求”，而是调度许多可独立失败的 part。状态机负责阶段，协调器负责并发与重试，服务端记录负责恢复事实；三者不能互相替代。
...
这里的“失败后不再调度”不等于瞬间取消所有已在飞行中的 part。若产品需要 fail-fast，应由协调器创建内部 AbortController，并把用户取消信号与内部失败信号组合；是否保留已成功 part 仍以后端事实为准。
...
可以自动重试的通常是明确的网络错误、408、429 和部分 5xx。常见的 4xx 表示认证、签名、请求格式或业务规则有问题，缺少 ETag 等协议错误也不会因重试自动修复。示例要求 transport 把真正的网络失败规范化为 `TypeError`，不再把所有没有 HTTP status 的普通 `Error` 都当成临时故障。
...
示例会校验 ID、时间、分片编号、ETag、重复项、记录年龄和数组上限；但它仍只是候选索引。账号切换或恢复前必须重新向服务端确认所有权和真实已完成分片。

#### tus 的 offset 模型

tus 以一个 upload resource URL 表示上传。客户端用 `HEAD` 查询服务端 `Upload-Offset`，再用带相同 offset 的 `PATCH` 继续；服务端只在字节被接受后推进 offset，offset 不匹配时应拒绝而不是覆盖。这与“客户端记住自己发到第几片”相比，把恢复事实放在服务端。

tus 与对象存储 multipart 不是同一协议。可以使用成熟实现或在后端做适配，不要把两者的 header、offset 和 complete 语义混拼。

## 从上传对象到可信资产

所有 part 成功只是对象合并的前置条件。接下来必须回答收到的字节是否完整、文件究竟是什么、是否安全，以及它能否进入当前业务。把这些问题拆开，才能避免用一个 ETag 或一次扫描包办所有保证。
...
最后把任务放回真实应用：路由会切换、多文件会争抢带宽、页面需要无障碍反馈，临时对象也会不断产生费用。这里关注所有权、调度、公平性和验证，而不是再增加协议名词。
...
<<< ../../../examples/frontend/file-upload-architecture/multipart-upload.test.ts
...
小文件上传不需要一开始就实现 multipart 和离线恢复。先把状态、信任边界和清理做对，再按照真实文件分布与失败率引入直传和分片，复杂度才有明确收益。

### 常见失败模式

#### 只相信 accept 和 File.type
...
下一节：[PWA、Service Worker、后台同步与离线应用架构](./pwa-service-worker-background-sync-and-offline-application-architecture.md)，会继续讨论页面离开或网络中断后，哪些资源和任务能够真正由浏览器后台能力接管。
