...
服务端事件还需要流序列、规范化后的文档和结构化拒绝原因：

<<< ../../../examples/frontend/realtime-collaboration/protocol.ts

示例只完整校验信封，并明确提醒生产环境要对每个 variant 使用共享运行时 schema。`as ServerEvent` 不会在运行时检查对象；消息可能来自旧服务、攻击者、损坏缓存或部署中的不同版本。
...
实际过期时间最好由服务端生成，避免客户端时钟差异。光标事件应节流或按帧合并，并按房间限制广播规模；presence 不是权限依据。
