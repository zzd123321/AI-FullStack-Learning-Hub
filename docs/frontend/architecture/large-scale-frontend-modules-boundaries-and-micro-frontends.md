...
```text
先从“一次变化影响了谁”识别业务边界
  → 在 Feature 内建立依赖方向和运行时解析边界
  → 选择调用、命令或事件完成跨边界协作
  → 用入口、Package exports、AST 检查和 CI 执行规则
  → 只有编译期边界不足以满足团队自治时，才评估微前端
```
...
示例让外部 JSON 先以 `unknown` 进入 `parsePublicRuntimeConfig()`，再校验 API URL 的协议、目录语义和 locale。TypeScript 接口不能证明部署平台实际注入了什么；如果跳过这一步，所谓“集中装配”仍把不可信字符串带进了整个对象图。
...
示例进一步要求 ID 非空，`occurredAt` 必须是 `toISOString()` 产生的规范 UTC 时间。明确一种 wire format 后，接收端不必猜测宽松日期字符串在不同时区和实现中的含义。

#### TypeScript 联合不能验证运行时消息
...
#### 不要用 TypeScript `paths` 伪造 Package
...
### TypeScript Project References 表达编译依赖图
...
下一节：[前端可观测性与生产治理](./frontend-observability-release-governance-and-incident-response.md)
