...
用户看到的是通知中心里的一张卡片，工程系统看到的却是一条跨越多个主体、可能在任意环节中断的异步链路。先分清能力和信任边界，后面才能正确理解“权限已授予”“订阅已创建”“Push Service 返回 201”和“用户已读”为什么是四件不同的事。
...
Web Push 的前半程发生在页面中：先让用户理解具体价值，再请求 origin 级通知权限，随后在某个 Service Worker registration 上创建设备订阅，最后把订阅与当前业务账号安全绑定。这个顺序不能倒过来。
...
示例要求调用者显式传入已确认可用的 `ServiceWorkerRegistration`，而不是在领域函数内部无限等待 `navigator.serviceWorker.ready`。页面可以先用 `getRegistration()` 判断当前 scope 是否已有注册；没有 active worker 时应显示“通知服务尚未就绪”或完成注册流程，而不是让按钮永久 loading。

先调用 `getSubscription()`，避免每次启动都无意义创建。示例还比较已有订阅绑定的 application server key；若 VAPID 公钥已轮换，会注销并删除旧记录，再创建新订阅。即使删除调用失败，失效 endpoint 以后也应通过 404/410 治理回收。无论复用还是新建，都应把当前 subscription 上报后端进行 reconciliation，因为服务端记录可能丢失、过期或绑定在旧账号。保存接口必须从当前服务端会话取得 user ID，不能相信客户端提交的 user ID。
...
订阅建立后，后端拿到的不是一个可以直接 `fetch(endpoint, { body: JSON.stringify(...) })` 的普通地址。发送端还要完成应用服务器认证、消息加密和 Web Push 协议头构造。这里最容易混淆的是 VAPID 与消息加密：二者都使用椭圆曲线密钥，但目的和密钥材料不同。
...
- RFC 8291 的消息加密会为发送消息使用独立密钥交换材料，不能把 VAPID 签名密钥直接当作内容加密密钥；
...
示例限制 schema 版本、长度、类别和同源相对 route，并在 URL 解析后比较 origin；这不仅拒绝 `//evil.example`，也防止 `/\\evil.example` 被特殊 URL scheme 将反斜杠归一化后变成跨源跳转。

## 在 Service Worker 中完成展示与安全点击

Push Service 接受消息后，浏览器可能在页面关闭时短暂启动 Service Worker。Worker 要在一次有限生命周期事件中解析不可信 payload、展示可见通知，并在以后另一次点击事件中恢复应用导航；任何模块级临时状态都不能作为这两步之间的桥梁。

### push 事件必须关联 waitUntil
...
客户端权限只回答这个 origin 能否展示通知，不回答某一条业务事件此刻是否值得打扰用户。类别偏好、静默时段、活跃状态、TTL、去重、限流和失效订阅治理必须在服务端发送管线中汇合，且在进入 Push Service 之前作出决定。
...
Web Push 使用 TTL、urgency、topic 等投递语义。RFC 8030 要求发送请求包含非负秒数的 TTL；它表示 Push Service 最多保留多久，服务仍可选择更短时间。urgency 影响服务在设备资源受限时的投递选择。topic 只替换**同一订阅上尚未投递**的同主题消息，最长 32 个 URL/文件名安全 Base64 字符，不是跨设备业务幂等键。具体 header 应由成熟库生成，并按业务验证。
...
示例关闭当前设备时先让服务端停止向该记录发送，再调用 `unsubscribe()`。这样即使浏览器侧注销失败，用户的“不再发送”意图也已经生效；后续 reconciliation 可以继续清理本地订阅。反过来先注销浏览器时，如果服务端删除失败，也可以依赖后续 404/410 回收，但停止发送会有不确定延迟。
...
至此技术链路已经闭环，但“能够发送”离“可以长期安全运营”还有很大距离。通知占用的是用户注意力，也可能在锁屏泄露内容；因此隐私、安全、可访问性、容量与运营约束都属于架构本身，而不是上线前附加的检查表。
...
<<< ../../../examples/frontend/web-push-notifications/worker-handlers.test.ts
...
下一节：[WebView、Electron 与跨端前端架构](./webview-electron-and-cross-platform-frontend-architecture.md)，会把运行环境从普通浏览器扩展到嵌入式 WebView 和桌面壳，继续处理能力边界、通信协议、导航安全、更新与发布。
