...
本课从浏览器媒体原语出发，再以 OpenAI Realtime API 为供应商适配案例。重点不是记住事件名称，而是建立媒体面、控制面、状态面与产品 UI 的正确边界。
...
示例用 wall-clock 演示契约，生产中应依据真正的播放时钟、队列与暂停时间计算 `audio_end_ms`。网络收到多少字节、解码了多少样本、audio 元素存在多久都不等于用户听了多久。
...
### 失败四：WebRTC 与 WebSocket 共用同一打断代码
...
### 失败六：每次 render 新建 PeerConnection

导致重复权限、连接和资源泄漏。连接由稳定 service/Provider 拥有。

### 失败七：日志上传完整 transcript 和 PCM

扩大隐私与安全面。默认只记录脱敏时序、错误和质量指标。

### 失败八：字幕 partial 不断 append
