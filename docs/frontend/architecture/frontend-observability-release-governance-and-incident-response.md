...
## 一、Monitoring 与 Observability 不完全相同
...
### 10.1 Framework Error Boundary 仍然必要
...
### 13.1 为什么不能向所有 Origin 注入 Header
...
## 十四、OpenTelemetry 应放在 Adapter 边界
...
### 15.1 Source Map 必须与 Release 精确对应
...
生产实现通常还要递归扫描 Chunk、校验 Release、上传 Debug ID、设置保留期，并在成功后从公开产物目录删除 `.map`。

## 十六、Release Marker 贯穿所有信号
...
配置下载包含某个 Flag，不代表用户真的进入了对应代码路径。示例在业务读取 Flag 时发送 `feature_flag.evaluated`，这样 Cohort 指标才有可信分母。
...
### 28.1 只接错误 SDK，不记录 Release

能看到异常，却无法判断是否由最新发布引入，也无法可靠匹配 Source Map。

### 28.2 把 `console.log` 当生产日志
...
### 28.5 Metric Label 使用 User ID 或完整 URL

高基数导致时间序列爆炸。需要用路由模板和有限 Cohort 聚合。

### 28.6 Source Map 和 Bundle 公开放在一起

调试方便，却无意公开原始源码内容。应上传到受控平台并关联 Release。

### 28.7 Flag 只负责打开，不负责删除
...
下一节建议进入**前端设计系统与跨框架组件平台**，系统讨论 Design Token、组件 API、可访问性、主题、版本兼容、文档、视觉回归和 Vue/React/Web Components 的共享边界。
