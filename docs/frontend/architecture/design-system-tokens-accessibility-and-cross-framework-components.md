...
设计系统不是组件文件集合，而是一套让产品界面持续保持一致、可访问和可演进的决策系统。它至少包含：
...
## 二、Design Token 保存“决策”，不是 CSS 缩写
...
## 三、三层 Token 模型表达不同稳定性

### 3.1 Core/Primitive Token
...
## 七、CSS Custom Property 是 Web 主题契约
...
真实 SSR 应尽量由服务器通过 Cookie 写出一致的 `data-theme`。若服务器不知道客户端偏好，可以让初始 CSS 使用 `prefers-color-scheme`，Hydration 后再同步用户显式选择。
...
还要处理 `storage` 或 BroadcastChannel 跨 Tab 同步、系统主题变化、无存储环境和 CSP Nonce。

## 九、主题不只有 Light 与 Dark
...
WCAG 2.2 还强调 Focus Appearance、Dragging Movements、Target Size 等新要求。设计系统可以集中解决大量基础问题，但“用了设计系统”不等于整个页面自动合规：页面结构、文案、焦点顺序和业务流程仍属于应用责任。
...
### 16.1 Attribute 与 Property 不能混为一谈
...
## 十七、CSS Parts 应像 Public API 一样克制
...
## 二十、SSR 与 Custom Element Upgrade
...
## 二十一、Form-associated Custom Element 要谨慎
...
Focus Indicator 必须在不同背景和主题中可见。不要全局 `outline: none`；如果使用自定义 Focus Style，需要验证面积、变化对比度和相邻对比度，而不是只在设计稿截图中看起来明显。
...
### 31.8 每次视觉 Diff 都更新基线

这会把回归正式记录为新标准。更新必须解释设计意图和消费者影响。

### 31.9 永不删除 Deprecated API
...
下一节建议进入**前端国际化与本地化工程**，系统讨论 Locale、消息格式、复数与性别、日期数字、时区、RTL、翻译资产、伪本地化、SSR 一致性和持续交付。
