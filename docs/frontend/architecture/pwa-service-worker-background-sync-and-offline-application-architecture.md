...
本课不会从“复制一个 cache-first 模板”开始，而是沿着因果顺序推进：先定义产品承诺与 Worker 生命周期，再决定哪些读取可以缓存；只有读路径可靠后，才加入 IndexedDB、Outbox 和后台同步，最后处理安装、配额、更新与回滚。
...
Service Worker 能拦截请求，却不知道产品希望哪些内容离线可用、能陈旧多久、哪些操作允许稍后同步。先把承诺写成用户可理解的能力等级，技术策略才有判断依据。
...
`updatefound` 还会发生在首次安装，只有已经存在 controller 时，新的 installed worker 才代表应用更新。`controllerchange` 可能触发多次，因此刷新需要一次性保护。示例还区分首次 `clients.claim()` 与真正版本接管：首次安装不应莫名刷新当前页，已有 controller 被替换或用户确认更新时才刷新。

更新提示应该可访问、可稍后处理，并说明刷新是否会影响未保存内容。

## 为不同读取选择缓存策略

页面受控后，每个匹配的 fetch 都可能经过 Worker。此时最危险的写法是“GET 就缓存”：请求方法相同，不代表数据的新鲜度、身份和公开性相同。先做明确路由，再实现策略。

### Cache API 不等于 HTTP Cache
...
- navigation：network first，离线时回退预缓存页面，但默认不保存任意 HTML；
- 带内容 hash 的静态资源：cache first；
- 明确位于 `/public-media/` 的公共图片：stale while revalidate；
- 其他请求：network only。

它刻意不缓存 API、头像和任意导航 HTML。生产中只有明确分析了身份、租户、敏感性、失效和容量后，才为某类资源添加规则。不能用 `/api/`、`destination: image` 之类宽泛条件代替数据分类。

`mayStore` 还拒绝 opaque、206、`no-store`、`private` 和 `Vary: *` 响应；它只是客户端最后一道保守判断，服务端仍应发送正确的 `Cache-Control`、认证和变化标识。
...
导航 fallback 从当前 worker 版本的 precache 中读取，而不是在同源所有 cache 中随意命中；否则激活交界期可能把旧离线 HTML 与新 worker、new chunks 拼成混合版本。

### 一个可审查的 Worker Runtime
...
示例在 activate 中调用 `clients.claim()`，并由页面注册控制器区分首次安装与更新刷新。因此部署前仍必须保证新 worker 的路由对当前已打开页面至少在刷新前向后兼容；如果不能保证，应去掉 claim，让新页面自然受控。
...
缓存响应解决的是“断网时还能读什么”，并没有解决用户修改。离线写入需要保存可重新认证、可去重、可检测冲突的业务意图；原始失败请求通常不具备这些性质。

### 离线读取：Cache Storage 还是 IndexedDB
...
每条 intent 带稳定 `principalId`，flush 只处理当前身份，避免账号切换后误发上一用户的操作；这里的 principal ID 是应用内部稳定标识，不是 access token。永久失败或达到重试预算后进入 dead letter，让用户修正、放弃或导出；可重试失败停止本轮顺序处理，避免后续依赖操作越过前一项。

`OutboxFlusher` 只合并同一身份、同一 JavaScript 上下文的并发 flush；窗口和 Service Worker 之间仍可能竞争，所以服务端幂等键不可省略。独立操作可以按实体分区并行，但必须由领域规则证明安全。
...
### Background Sync 是增强能力

Background Sync 可以注册 tag，让浏览器在认为网络恢复时唤醒 Service Worker 处理 `sync` 事件。但它不是所有主流浏览器都支持，也不保证在特定时刻执行。

因此架构必须先有前台 flush：应用启动、回到前台、用户手动重试或 `online` 事件时尝试同步；支持 Background Sync 的浏览器再额外注册。注册成功也不是执行保证，不能因此移除前台监听。

<<< ../../../examples/frontend/pwa-offline-architecture/sync-scheduler.ts

调度器始终保留前台 `online` 路径，并使用 `getRegistration()` 避免在根本没有 active worker 时永久等待 `navigator.serviceWorker.ready`。Worker runtime 则监听 `sync` tag，并把 flush Promise 立即传给 `event.waitUntil()`；只注册 tag 而没有处理器不会完成任何同步。
...
离线读写完成后，manifest 才把网站连接到系统安装体验。安装不会增强数据一致性，也不会赠送永久存储；平台能力、配额和用户授权仍可能随时缺失。
...
Service Worker 往往比页面代码存活更久，真实生产环境会长期存在 v1 页面、v2 worker、旧 IndexedDB 和新 API 的组合。因此构建产物、运行时协议、测试与回滚必须共同设计。
...
<<< ../../../examples/frontend/pwa-offline-architecture/worker-runtime.test.ts
...
多数站点的第一步只需要可靠更新和一个离线提示。只有当用户确实需要离线阅读或编辑时，再引入运行时缓存和 Outbox，避免为了“PWA”标签承担无收益的分布式状态复杂度。
...
下一节：[Web Push、通知权限与后台消息架构](./web-push-notification-permission-and-background-messaging-architecture.md)，会继续使用 Service Worker 处理服务器主动消息、通知权限、订阅生命周期和点击路由。
